				      ...
				      struct wpa_ssid *ssid)
{
	...
	for (profile = 0; profile <= SEC_PROF_MAX && profile < bitmap_len * 8;
...
#ifdef CONFIG_PQC
		{
			...
			/*
			 * Skip profiles whose PQC constraint is not in the
			 * configured supported_pqc_constraints.
			 */
...
int security_profile_get_key_mgmt(const u8 *sp, struct wpa_ssid *ssid)
{
	...
	return security_profile_get_key_mgmt_akm(sp, ssid->key_mgmt, ssid);
}
			      ...
			      bool skip_default_rsne)
{
	...
	if (sel_sp) {
		...
		wpas_ssid_pqc_constraints(ssid, &pqc_constraints,
					  &num_pqc_constraints);
...
static bool wpas_pqc_akm_configured(struct wpa_supplicant *wpa_s)
{
	...
	for (ssid = wpa_s->conf->ssid; ssid; ssid = ssid->next) {
		if (!ssid->disabled && wpa_key_mgmt_pqc(ssid->key_mgmt))
			return true;
	}

				  ...
				  const u8 *buf, size_t len, u8 *mic)
{
	...
	if (hash_alg == RSN_HASH_NOT_SPECIFIED) {
		if (key_len == 128 / 8)
			hash_alg = RSN_HASH_SHA256;
		else if (key_len == 192 / 8)
			hash_alg = RSN_HASH_SHA384;
		else if (key_len == 256 / 8)
			hash_alg = RSN_HASH_SHA512;
		else
			return -1;
	}

	wpa_printf(MSG_DEBUG,
		   "RSN: EAPOL-Key MIC using HMAC-SHA%u (AKM-defined - %s)",
		   hash_alg == RSN_HASH_SHA512 ? 512 :
		   hash_alg == RSN_HASH_SHA384 ? 384 : 256,
		   wpa_key_mgmt_txt(akmp, WPA_PROTO_RSN));
	wpa_hexdump(MSG_DEBUG, "RSN: KCK", key, key_len);

	if (hash_alg == RSN_HASH_SHA256) {
...
#endif /* CONFIG_SHA384 */
#ifdef CONFIG_SHA512
	} else if (hash_alg == RSN_HASH_SHA512) {
		...
		wpa_printf(MSG_INFO,
			   "RSN: Unsupported KCK length: %u (hash_alg=%d)",
			   (unsigned int) key_len, hash_alg);
		return -1;
	}
...
#endif /* CONFIG_SAE || CONFIG_OWE || CONFIG_DPP */
...
security_profile_table[] = {
	...
	{ SEC_PROF_8021X_PQC_0, WPA_KEY_MGMT_802_1X_PQC, WPA_CIPHER_GCMP_256,
	  true, true, true, PQC_CONSTRAINT_ML_KEM_1024 },

	/* 17: PQC 802.1X (AKM 31), P-256, SHA-256, ML-KEM-512 */
	{ SEC_PROF_8021X_PQC_1, WPA_KEY_MGMT_802_1X_PQC, WPA_CIPHER_GCMP_256,
	  true, true, true, PQC_CONSTRAINT_ECP_19_ML_KEM_512 },

	/* 18: PQC 802.1X (AKM 31), P-384, SHA-384, ML-KEM-768 */
	{ SEC_PROF_8021X_PQC_2, WPA_KEY_MGMT_802_1X_PQC, WPA_CIPHER_GCMP_256,
	  true, true, true, PQC_CONSTRAINT_ECP_20_ML_KEM_768 },

	/* 19: PQC 802.1X (AKM 31), P-521, SHA-512, ML-KEM-1024 */
	{ SEC_PROF_8021X_PQC_3, WPA_KEY_MGMT_802_1X_PQC, WPA_CIPHER_GCMP_256,
	  true, true, true, PQC_CONSTRAINT_ECP_21_ML_KEM_1024 },

	/* 20: FT PQC 802.1X (AKM 32), no ECP, SHA-512, ML-KEM-1024 */
	{ SEC_PROF_8021X_FT_PQC_0, WPA_KEY_MGMT_FT_802_1X_PQC,
	  WPA_CIPHER_GCMP_256, true, true, true, PQC_CONSTRAINT_ML_KEM_1024 },

	/* 21: FT PQC 802.1X (AKM 32), P-256, SHA-256, ML-KEM-512 */
	{ SEC_PROF_8021X_FT_PQC_1, WPA_KEY_MGMT_FT_802_1X_PQC,
	  WPA_CIPHER_GCMP_256, true, true, true,
	  PQC_CONSTRAINT_ECP_19_ML_KEM_512 },

	/* 22: FT PQC 802.1X (AKM 32), P-384, SHA-384, ML-KEM-768 */
	{ SEC_PROF_8021X_FT_PQC_2, WPA_KEY_MGMT_FT_802_1X_PQC,
	  WPA_CIPHER_GCMP_256, true, true, true,
	  PQC_CONSTRAINT_ECP_20_ML_KEM_768 },

	/* 23: FT PQC 802.1X (AKM 32), P-521, SHA-512, ML-KEM-1024 */
	{ SEC_PROF_8021X_FT_PQC_3, WPA_KEY_MGMT_FT_802_1X_PQC,
	  WPA_CIPHER_GCMP_256, true, true, true,
	  PQC_CONSTRAINT_ECP_21_ML_KEM_1024 },
#endif /* CONFIG_PQC */
};
